...
export default function EditScreeningQuestionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);
    const opts = options.map((s) => (typeof s === "string" ? s.trim() : "")).filter(Boolean);
    if (!title.trim()) {
    ...
    const correct = correctIndex < 0 || correctIndex >= opts.length ? 0 : correctIndex;
    setSubmitting(true);
    try {
      const res = await fetch(`/api/admin/screening/questions/${id}`, {
        method: "PATCH",
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <Link href="/admin/dashboard/screening/questions" className="inline-flex items-center gap-2 text-sm text-[var(--text-muted)] hover:text-[var(--accent-blue)]">
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="max-w-xl space-y-4">
        ...
        <div>
          ...
          <button type="button" onClick={addOption} className="text-sm text-[var(--accent-blue)] hover:underline">
            + Add option
          </button>
        </div>
        <div>
